...
<div class="w3-main" style="margin-left:250px">
  ...
  <div class="w3-row w3-padding-64">
    <div class="w3-thirdfour w3-container">
        <p class="ml3" style="margin-left:20px">科系管理
        @php
            if($list != [])
                echo "<button style='float:right;' class='update' onclick='open_window();'>刪除科系</button>
                      </p><br>";
        @endphp
        ...
        <div style="background-color:MintCream;">
            <br>
            <center><h5 style="color:#556B2F;">科系
            <input style="margin-left:10px" type="text" placeholder=" 系名/系碼" name="dp">
            <button style="margin-left:10px" class="submit">查詢</button></center></h5>
            <br>
        </div>
      </form>
        <div style="background-color:MintCream;">
          <form action="/admin/Department_origin" method="POST">
            @csrf
            @php
                echo "<center><table>
                      <thead><tr>
                ...
                foreach($list as $l){
                    echo "<tr>
                          ...
                          <td>$l->DP_num</td>
                          <td>$l->Dean</td>
                          <td><button class='update' name='origin' value=$l->DP_num>修改</button></td>";
                }
                echo "</tr></tbody></table></center>
                      <br><center>";
                echo "<button style='margin-left:20px' name='origin' value='null' class='submit'>新增</button>
                      </center><br>";
            @endphp
          </form>
        </div>
        <div class='popup' style='display: none;' id='popWindow_2'>
            ...
            <center>
            <br>
            <form action="/admin/Department_drop" method="POST">
            @csrf
                <h5 style='color:#556B2F;'>科系
                <input style='margin-left:10px' type='text' placeholder='輸入待刪除的科系代碼' name='DP_num'></h5>
                </center>
                <br>
          ...
          @endphp
          <form action="/admin/Department_update"  method="POST" class="form-container">
          @csrf
          @php
            if($popup == "1" ){
                if($current != null)
                    echo" <input type='hidden' name='origin' value=$current->DP_num>
                          <h5 style='color:#556B2F;'>科系
                          <input style='margin-left:10px' type='text' placeholder=$current->DP name='DP'></h5>
                          <h5 style='color:#556B2F;'>系碼
                          <input style='margin-left:10px' type='text' placeholder=$current->DP_num name='DP_num'></h5>
                          <h5 style='color:#556B2F;'>系主任
                          <input style='margin-left:10px' type='text' placeholder=$current->Dean name='Dean'></h5>";
                else
                    echo" <input type='hidden' name='origin' value='null'>
                          <h5 style='color:#556B2F;'>科系
                          <input style='margin-left:10px' type='text' placeholder='科系名稱' name='DP'></h5>
                          <h5 style='color:#556B2F;'>系碼
                          <input style='margin-left:10px' type='text' placeholder='科系代碼' name='DP_num'></h5>
                          <h5 style='color:#556B2F;'>系主任
                          <input style='margin-left:10px' type='text' placeholder='姓名' name='Dean'></h5>";
                echo"</center><br>
                     <button class='submit'>確認</button>
                     <br></h5></div>";
            }
