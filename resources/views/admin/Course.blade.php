...
<div class="w3-main" style="margin-left:250px">
  ...
  <div class="w3-row w3-padding-64">
    <div class="w3-thirdfour w3-container">
        <p class="ml3" style="margin-left:20px">課程管理
        @php
            if(!$list->isEmpty())
                echo "<button style='float:right;' class='update' onclick='open_window();'>刪除課程</button>
                      </p><br>";
        @endphp
      <form action="/admin/Course" method="POST">
        ...
        if(!$list->isEmpty()){
            echo "<div style='background-color:MintCream;'>
                <br>
                <center><h5 style='color:#556B2F;'>課程
                <input style='margin-left:10px' type='text' placeholder=' 課程名/課程號' name='course'>
        ...
        <div style="background-color:MintCream;">
          <form action="/admin/Course_origin" method="POST">
            @csrf
            @php
            if($list->isEmpty()){
                echo "<br><br><center><h4 style='color:#455A64'>暫無課程</h4></center><br>";
            }
            else{
                echo "<center><table>
                      <thead><tr>
                ...
                foreach($list as $l){
                    echo "<tr>
                          ...
                          <td>$l->CS_num</td>
                          <td>$l->Credit</td>
                          <td><button class='update' name='origin' value=$l->CS_num>修改</button></td>";
                }
        ...
        <div class='popup' style='display: none;' id='popWindow_2'>
            ...
            <center>
            <br>
            <form action="/admin/Course_drop" method="POST">
            @csrf
                <h5 style='color:#556B2F;'>課程
                <input style='margin-left:10px' type='text' placeholder='輸入待刪除的課程代號' name='CS_num'></h5>
                </center>
                <br>
          ...
          @php
            if($popup == "1" ){
                echo"<div class='popup' id='popWindow_1'>
                     <button id='close'class='cancel' onclick='close_window();'>&times;</button>
                     <center><br>";
            }
          @endphp
          <form action="/admin/Course_update"  method="POST" class="form-container">
          @csrf
          @php
            if($popup == "1" ){
                if($current != null)
                    echo" <input type='hidden' name='origin' value=$current->CS_num>
                          <h5 style='color:#556B2F;'>課程
                          <input style='margin-left:10px' type='text' placeholder=$current->CS name='CS'></h5>
                          <h5 style='color:#556B2F;'>課號
                          <input style='margin-left:10px' type='text' placeholder=$current->CS_num name='CS_num'></h5>
                          <h5 style='color:#556B2F;'>學分
                          <input style='margin-left:10px' type='text' placeholder=$current->Credit name='Credit'></h5>";
                else
                    echo" <input type='hidden' name='origin' value='null'>
                          <h5 style='color:#556B2F;'>課程
                          <input style='margin-left:10px' type='text' placeholder='課程名稱' name='CS'></h5>
                          <h5 style='color:#556B2F;'>課號
                          <input style='margin-left:10px' type='text' placeholder='課程代碼' name='CS_num'></h5>
                          <h5 style='color:#556B2F;'>學分
                          <input style='margin-left:10px' type='text' placeholder='學分數' name='Credit'></h5>";
                echo"</center><br>
                     <button class='submit'>確認</button>
                     <br></h5></div>";
            }
